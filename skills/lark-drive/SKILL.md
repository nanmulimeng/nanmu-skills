...
name: lark-drive
version: 1.0.0
description: "飞书云空间操作：管理云空间中的文件和文件夹，包括上传/下载/复制/移动/删除文件、创建文件夹、管理文档评论和权限、修改文件标题、导入本地文件为在线文档（docx/sheet/bitable）。通过 lark-cli 命令操作飞书云空间。当用户需要上传或下载文件、整理云空间目录、管理评论和权限，或要把本地文件导入成在线文档时使用。即使用户只是说'上传文件到飞书'或'整理云空间'，也应触发。"
metadata:
  requires:
    bins: ["lark-cli"]
