...
name: lark-task
version: 1.0.0
description: "飞书任务操作：管理任务和清单，包括创建待办任务、查看和更新任务状态、拆分子任务、组织任务清单、分配协作成员。通过 lark-cli 命令操作飞书任务功能。当用户需要创建待办事项、查看任务列表、跟踪任务进度、管理项目清单或给他人分配任务时使用。即使用户只是说'创建待办'或'查下我的任务'，也应触发。如果未自动触发，手动写：`使用 lark-task`。"
metadata:
  requires:
    bins: ["lark-cli"]
