...
<!-- DEPS_CHECK: scripts/check-deps.mjs 尚未创建，跳过自动检查 -->
<!-- 手动确认以下依赖后继续 -->

If check unavailable, guide user to set up:
...
<!-- DEPS_CHECK: scripts/check-deps.mjs 尚未创建，手动确认后继续 -->
<!-- 手动确认：Node.js 22+ 已安装、Chrome 远程调试端口已开启、Proxy 已连接 -->

Ensure manually: Node.js 22+ is installed, Chrome remote debugging port is open, and Proxy is connected (auto-start if not running, wait for ready). After Proxy starts, it keeps running.
...
Accumulated experience for specific websites. <!-- 注：`references/site-patterns/` 目录尚未创建，后续可按需建立 -->
