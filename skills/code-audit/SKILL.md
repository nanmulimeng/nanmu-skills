---
name: code-audit
description: 代码审计技能 - 安全审计、代码质量审查、漏洞挖掘、SAST。当你涉及安全检查、漏洞修复、权限认证逻辑修改、密钥管理、输入验证消毒、SQL注入/XSS/CSRF/IDOR/SSRF修复、反序列化安全、加密算法审计、依赖CVE审计、CWE映射、OWASP Top 10合规时必须使用此技能。即使用户只是说"检查下安全"、"加个权限判断"或"审计一下"，也应触发。如果未自动触发，手动写：`使用 code-audit`。
---
