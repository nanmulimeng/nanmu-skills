...
name: lark-minutes
version: 1.0.0
description: "飞书妙记操作：搜索妙记列表、获取妙记基础信息（标题/封面/时长）、下载妙记音视频文件。通过 lark-cli 命令操作飞书妙记。当用户需要搜索妙记、查看妙记信息、下载妙记音视频时使用。即使用户只是说'搜下妙记'或'下载妙记视频'，也应触发。如果未自动触发，手动写：`使用 lark-minutes`。"
metadata:
  requires:
    bins: ["lark-cli"]
