...
name: lark-doc
version: 1.0.0
description: "飞书云文档操作：创建和编辑飞书文档，从 Markdown 创建文档、获取/更新文档内容、上传下载文档中的图片和文件、搜索云空间文档。通过 lark-cli 命令操作飞书云文档。当用户需要创建或编辑飞书文档、读取文档内容、在文档中插入图片、搜索云空间文档时使用；如果用户是想按名称定位电子表格等云空间对象，也优先使用本 skill。即使用户只是说'创建个文档'或'搜下飞书文档'，也应触发。如果未自动触发，手动写：`使用 lark-doc`。"
metadata:
  requires:
    bins: ["lark-cli"]
