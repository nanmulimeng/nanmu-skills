...
name: lark-wiki
version: 1.0.0
description: "飞书知识库操作：管理知识空间、空间成员和文档节点（创建/查询/移动/复制）。通过 lark-cli 命令操作飞书知识库。当用户需要在知识库中查找或创建文档、管理空间成员、移动或复制节点时使用。即使用户只是说'查下知识库'或'添加知识库成员'，也应触发。如果未自动触发，手动写：`使用 lark-wiki`。"
metadata:
  requires:
    bins: ["lark-cli"]
