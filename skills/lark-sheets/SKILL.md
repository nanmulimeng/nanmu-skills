...
name: lark-sheets
version: 1.1.0
description: "飞书电子表格操作：创建和操作电子表格，包括读写单元格、追加行、查找替换、导出下载、管理筛选视图、设置样式、插入浮动图片。通过 lark-cli 命令操作飞书电子表格。当用户需要创建电子表格、批量读写数据、在表格中查找内容、导出表格文件时使用。即使用户只是说'创建个表格'或'更新表格数据'，也应触发。如果未自动触发，手动写：`使用 lark-sheets`。"
metadata:
  requires:
    bins: ["lark-cli"]
