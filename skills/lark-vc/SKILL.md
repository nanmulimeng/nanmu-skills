...
name: lark-vc
version: 1.0.0
description: "飞书视频会议操作：查询已结束的会议记录、获取会议纪要产物（总结、待办、章节、逐字稿）。支持按关键词、时间范围、组织者、参与者、会议室等筛选搜索。通过 lark-cli 命令操作飞书视频会议。当用户需要查询历史会议、搜索会议记录、获取整理会议纪要时使用。即使用户只是说'查历史会议'或'获取会议纪要'，也应触发。"
metadata:
  requires:
    bins: ["lark-cli"]
