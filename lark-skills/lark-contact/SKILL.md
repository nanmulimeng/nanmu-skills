...
name: lark-contact
version: 1.0.0
description: "飞书通讯录操作：查询组织架构、人员信息和搜索员工。获取当前用户或指定用户的详细信息、通过关键词搜索员工（姓名/邮箱/手机号）。通过 lark-cli 命令操作飞书通讯录。当用户需要查看个人信息、查找同事联系方式、按姓名搜索员工、查询部门结构时使用。即使用户只是说'搜下同事'或'查组织架构'，也应触发。如果未自动触发，手动写：`使用 lark-contact`。"
metadata:
  requires:
    bins: ["lark-cli"]
