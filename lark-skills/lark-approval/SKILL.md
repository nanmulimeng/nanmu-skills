...
name: lark-approval
version: 1.0.0
description: "飞书审批操作：查询审批实例详情、管理审批任务（同意/拒绝/转交/撤回/催办）、查看已发起和待审批列表。通过 lark-cli 命令操作飞书审批功能。当用户需要处理审批、查看审批状态、同意或驳回审批、撤回已提交的审批时使用。即使用户只是说'帮我审批一下'或'查审批进度'，也应触发。如果未自动触发，手动写：`使用 lark-approval`。"
metadata:
  requires:
    bins: ["lark-cli"]
